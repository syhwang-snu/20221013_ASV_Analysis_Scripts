...
gtf2exon <- function(ens_gtf){
    ...
    ens_gtf_exons$seqnames <- ens_gtf_exons$seqid
...
geneTranscriptPlot <- function(g = 'Atp6v0a1' , x = topGenes_table, gtf_exon_table = ens_gtf_exons, CPM_columns = c('Ctrl','PA'), rescaled = TRUE, cpm_y_all){
    ...
    # CPM_columns : column names that contain CPM values
    
    g_df <- gtf_exon_table %>% dplyr::filter(gene_name == g)
    g_df <- g_df %>% left_join(x[,c('transcript_id','logFC',CPM_columns)], by=c('transcript_id'))
    ...
    g_cpms <- cpm_y_all %>% dplyr::select(gene_name, transcript_name, {{col1}}, {{col2}}) %>% 
        dplyr::filter(gene_name == g)
    print(head(g_cpms))
    
    g_cpms<- g_cpms %>% group_by(gene_name) %>% 
        pivot_longer(cols = c(all_of(col1),all_of(col2)), names_to = "Sample",values_to = "CPM")  %>% 
